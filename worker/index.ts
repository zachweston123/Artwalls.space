...
<body>
  ...
  <div class="url">Scan to view &amp; purchase</div>
    ...
    // Returns an XML sitemap that includes static marketing pages, all city
    // pages, public artist profiles, public artworks, and public venue profiles.
    // Only exposes data that is already publicly accessible via the app.
    // ══════════════════════════════════════════════════════════════════════════
    ...
    if (url.pathname === '/sitemap.xml' && method === 'GET') {
      ...
      // ── City pages (hardcoded list — matches frontend src/data/cities.ts) ──
      const citySlugs = [
      ...
      if (supabaseAdmin) {
        // Public artist profiles (with slug)
        try {
          const { data: artists } = await supabaseAdmin
            .from('artists')
        ...
        // Public artworks (available/active, not archived)
        try {
          const { data: artworks } = await supabaseAdmin
            .from('artworks')
