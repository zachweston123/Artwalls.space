...
export function VenueAnalytics({ user }: VenueAnalyticsProps) {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      // Fetch orders for this venue in the time range
      const { data: orders, error: ordersErr } = await supabase
        .from('orders')
        .select('id, artwork_id, amount_cents, venue_commission_cents, created_at')
        .eq('venue_id', user.id)
      ...
      for (const o of orderList) {
        ...
        existing.earningsCents += o.venue_commission_cents || 0;
      ...
      if (artworkIds.length > 0) {
        ...
        for (const a of artworks || []) {
          artworkInfo.set(a.id, { title: a.title || 'Untitled', artistName: a.artist_name || 'Unknown' });
        }
      }
    ...
    <PageShell size="wide" className="text-[var(--text)]">
      ...
      {!loading && !error && (
        <>
          ...
          <SectionCard
            title="Top Artworks"
            subtitle="Pieces at your venue ranked by total engagement (scans + views + sales)."
          >
