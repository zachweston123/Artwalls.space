...
export function ArtistAnalytics({ user }: ArtistAnalyticsProps) {
  ...
  return (
    <div className="bg-[var(--bg)] text-[var(--text)]">
      <h1 className="text-2xl mb-4">Artist Analytics</h1>
      ...
      <div className="bg-[var(--surface-2)] border border-[var(--border)] rounded-xl p-4">
        <h2 className="text-sm mb-3">Per Artwork</h2>
        <div className="space-y-2 text-sm">
          {data?.perArtwork ? (
            Object.entries(data.perArtwork).map(([id, stats]: any) => (
              <div key={id} className="flex justify-between">
                <span className="text-[var(--text-muted)]">{id.slice(0, 8)}</span>
                <span>Scans {stats.scans} • Views {stats.views} • Checkouts {stats.checkouts} • Purchases {stats.purchases}</span>
              </div>
            ))
          ) : (
